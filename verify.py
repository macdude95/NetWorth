...
def verify():
    ...
    with sync_playwright() as p:
        ...
        page.locator("#scaleZero").click()
        check(page.evaluate("Chart.getChart(document.getElementById('chartNetWorth')).options.scales.y.min === 0 && Chart.getChart(document.getElementById('chartProjections')).options.scales.y.min === 0"), "zero chart scale restores zero floor")
        ...
        check(page.evaluate("(() => { const m = buildMilestones(projCache).find(x => x.name === '5-year bridge'); const p = DATA.retirement_profile; const e = DATA.current.expenses / 12; const r = 7 / 100 / 12; const i = p.inflation_rate / 100 / 12; const t = (p.birth_year + p.target_age - 2026) * 12 + (p.birth_month - 9); let expected = 0; for (let k = 1; k <= 60; k++) expected += e * Math.pow(1 + i, t + k - 1) / Math.pow(1 + r, t + k); return !!m && Math.abs(m.value - expected) < 1; })()"), "bridge targets use inflation and expected return")
        check(page.evaluate("(() => { const a = buildMilestones({...projCache, annualGrowth: 4}).find(x => x.name === '5-year bridge').value; const b = buildMilestones({...projCache, annualGrowth: 9}).find(x => x.name === '5-year bridge').value; return a === b; })()"), "bridge targets ignore selected scenario")
        ...
        check("e million" in page.locator(".achievement-ladder").nth(4).inner_text(), "fun e milestone renders")
        check(page.locator("text=$1,234,567 liquid").count() >= 1, "fun sequential number milestone renders")
        check(page.evaluate("JSON.stringify([...document.querySelectorAll('.achievement-ladder')[4].children].map(x => x.textContent.trim())) === JSON.stringify(['◆ $1,234,567', '· φ million', '· e million', '· π million'])"), "fun milestones sort least to greatest")
        ...
        check(page.evaluate("Math.abs(buildMilestones(projCache).find(x => x.name === 'e million liquid').value - Math.E * 1000000) < 0.01"), "fun e milestone uses Euler target")
        ...
        page.locator("#trophyCase summary").click()
        check(page.locator("#trophyCase").get_attribute("open") is not None and page.locator(".trophy-chip").count() >= 1, "trophy case browses collected achievements")
